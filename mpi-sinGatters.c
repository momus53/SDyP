...
int main(int argc, char*argv[]){
    ...
    for (int lenTrabajo = 4; lenTrabajo <= N; lenTrabajo *= 2) {
        ...
        if (lenTrabajo>=elementos_por_proc && elementos_por_proc < N){
            ...
            if(elementos_por_proc <= n_local && rank != 0){ // solo entran los hilos que todavia tienen trabajo
                MPI_Send(a, elementos_por_proc, MPI_INT, rank-1, MPI_ANY_TAG, MPI_COMM_WORLD); //SE PISAN LOS ARREGLOS viejos o QUE ONDA??
            }else{ //No tengo mas trabajo que hacer.
              if(rank != 0) break; // el 0 siempre tiene trabajo que hacer.
            }
            
            MPI_Recv (a_local, elementos_por_proc, MPI_INT, rank+1 , MPI_ANY_TAG, MPI_COMM_WORLD); //recibo de mi vecino
            //deberia escribir los datos nuevos que recibo en "a" acá?
            
            if (elementos_por_proc > N && rank == 0){
                    a_local_bk = a_local;
                    b_local_bk = b_local;
