...
int resultado = 0;
int rank;
int *aux_local, *a_local, *b_local, *a, *b, *a_bk, *b_bk;
    ...
    */
    ...
    for (int lenTrabajo = 4; lenTrabajo <= N; lenTrabajo *= 2) {
      ...
      if (elementos_por_proc <= n_local) {  
        ...
        if (lenTrabajo>=elementos_por_proc && elementos_por_proc < N){
            //printf("Soy rank %d y voy a hacer gather de a con %d elementos_por_proc \n", rank,elementos_por_proc);
            //MPI_Barrier(MPI_COMM_WORLD);
            ...
            //if (elementos_por_proc > n_local && elementos_por_proc != 0){
                ////printf("Soy rank %d y elementos_por_proc es mayor a n_local, por lo que me piro!!!!\n", rank);
            ...
            if (elementos_por_proc < N){
                ...
                if (rank == 0) {
                    //printf("Soy rank %d y elementos_por_proc es mayor o igual a N, por lo que a_local es a\n", rank);
                    a_local = a;
                }
            }
        }
    ...
    for (int lenTrabajo = 4; lenTrabajo <= N; lenTrabajo *= 2) {
      ...
      if (elementos_por_proc <= n_local) {  
        ...
        //}
        if (lenTrabajo>=elementos_por_proc && elementos_por_proc < N){
            //printf("Soy rank %d y voy a hacer gather de b con %d elementos_por_proc \n", rank,elementos_por_proc);
            //MPI_Barrier(MPI_COMM_WORLD);
    ...
    if (rank == 0) {
        //printf("Soy rank %d, hemos terminao por ahora, el arreglo b es: \n", rank);
        b = b_local;
                    for (i = 0; i < N; i++) {
                        //printf("b[%d] es %d\n", i, b[i]);
                    }
